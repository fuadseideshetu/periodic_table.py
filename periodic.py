...
def get_element(query):
    for number, info in periodic_table.items():
        if str(number) == str(query) or info["symbol"].lower() == str(query).lower() or info["name"].lower() == str(query).lower():
            return f"Atomic Number: {number}, Symbol: {info['symbol']}, Name: {info['name']}"
    return "Element not found."

# Example usage:
while True:
    user_input = input("\nEnter atomic number, symbol, or name (or 'exit' to quit): ")
    if user_input.lower() == "exit":
        break
    print(get_element(user_input))
